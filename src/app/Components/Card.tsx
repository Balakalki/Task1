...
const JobCardCom: React.FC<JobCardProps> = ({ jobData }) => {
  ...
  const timeAgo = dayjs.utc(jobData.created_at).local().fromNow().replace(" minutes", "m").replace(" minute", "m").replace(" hours", "h").replace(" hour", "h").replace(" days", "d").replace(" day", "d").replace(" month", "m").replace(" year", "y").replace("ad","1d").replace("anh","1h").replace("am","1m").replace("ay","1y").replace("ago", "Ago");
